	...
	if (items_valid_) {
		...
		case CONTEXT_CELL:
			...
			fl_push_clip(X, Y, W, H);
			{
				...
				if (type_ == VT_MEMORIES && **(items_ + R) == "NO SPLIT") {
					fl_color(FL_GRAY);
				}
				else {
...
void view73::draw_user_bands_view() {
	...
	for (int i = 0; i < num_txbands && ok; i++) {
		...
		*(items_ + r) = item;
		...
		string tx_lower = bcd_to_string(raw_tx_band.substr(0, 5), 6, false);
		string tx_upper = bcd_to_string(raw_tx_band.substr(6, 5), 6, false);
		item[0] = tx_lower;
		item[1] = tx_upper;
		...
		while (user_valid && user < num_userbands) {
			...
			if (raw_user_band[0] == '\xff' || raw_user_band.length() != 11) {
				...
				string user_lower = bcd_to_string(raw_user_band.substr(0, 5), 6, false);
				string user_upper = bcd_to_string(raw_user_band.substr(6, 5), 6, false);
