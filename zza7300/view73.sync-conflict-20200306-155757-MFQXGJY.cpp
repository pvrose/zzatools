...
void view73::draw_memory_view() {
	...
	for (int i = 1; i <= rows() && ok; i++) {
		...
		if (data.length() != 0) {
			...
			item[2] = to_string(bcd_to_double(data.substr(1, 5), 6, false));
			// Operating mode
			switch (data[6]) {
			case 0:
				item[3] = "LSB";
			...
			double d;
			d = bcd_to_double(data.substr(9, 3), 1, true);
			char temp[10];
			sprintf(temp, "%.1f", d);
			item[6] = d;
			// Tone squelch settin
			d = bcd_to_double(data.substr(12, 3), 1, true);
			sprintf(temp, "%.1f", d);
			item[7] = temp;
			// Split frequency (or common frequency)
			item[8] = to_string(bcd_to_double(data.substr(15, 5), 6, false));
			// Operating mode
			switch (data[20]) {
			case 0:
				item[9] = "LSB";
			...
			// REpeater tone setting
			d = bcd_to_double(data.substr(23, 3), 1, true);
			sprintf(temp, "%.1f", d);
			item[12] = d;
			// Tone squelch settin
			d = bcd_to_double(data.substr(26, 3), 1, true);
			sprintf(temp, "%.1f", d);
			item[13] = temp;
			// Mmoory name
...
void view73::draw_scope_bands_view() {
	...
	for (int r = 0; r < rows() && ok; r++) {
		...
		string* item = new string[cols()];
		double d = bcd_to_double(data.substr(0, 3), 6, true);
		char temp[10];
		sprintf(temp, "%.6f", d);
		item[0] = d;
		// Tone squelch settin
		d = bcd_to_double(data.substr(3, 3), 6, true);
		sprintf(temp, "%.6f", d);
		item[1] = temp;
		*(items_ + r) = item;
	}
