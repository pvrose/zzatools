...
static void cb_value_tm(Fl_Widget* w, void* v) {
	...
	zzalib::string_to_tm(string(value), *target, "%Y%m%d");
}
...
static void cb_bn_browsefile(Fl_Widget* w, void* v) {
	...
	Fl_File_Chooser* chooser = new Fl_File_Chooser(data->filename->c_str(), data->pattern.c_str(), Fl_File_Chooser::CREATE, data->message.c_str());
	chooser->callback(cb_chooser, data->filename);
	...
	if (chooser->count()) {
		// If at least 1 file is selected - write filename to input widget
...
static void cb_bn_browsedir(Fl_Widget* w, void* v) {
	browser_data_t* data = (browser_data_t*)v;
	// Open file chooser with supplied data
	Fl_File_Chooser* chooser = new Fl_File_Chooser(data->filename->c_str(), "", Fl_File_Chooser::DIRECTORY, data->message.c_str());
	chooser->callback(cb_chooser, data->filename);
	chooser->textfont(FONT);
	chooser->textsize(FONT_SIZE);
	chooser->show();
	...
	while (chooser->visible()) Fl::wait();
	if (chooser->count()) {
		// If at least 1 directory is selected - write filename to input widget
		(data->input_w)->value(data->filename->c_str());
		if (data->enable != nullptr) {
			// If a check widget and bool is supplied, set it
			*(data->enable) = true;
