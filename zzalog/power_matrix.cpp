...
power_matrix::power_matrix(string rig)
{
	...
	if (rig_settings.groupExists("Power Matrix")) {
		...
		for (int g = 0; g < matrix_settings.groups(); g++) {
			...
			map<int, int>* power_map = new map<int, int>;
			for (int e = 0; e < band_settings.entries(); e++) {
				...
				int drive;
				band_settings.get(name.c_str(), drive, -1);
				// Map power to drive
				(*power_map)[power] = drive;
			}
			map_[band] = power_map;
		}
	}
...
power_matrix::~power_matrix()
{
	...
	for (auto it = map_.begin(); it != map_.end(); it++) {
		...
		map <int, int>* power_map = (map <int, int>*) it->second;
...
int power_matrix::power(string band, int drive) {
	if (map_.find(band) != map_.end()) {
		map <int, int>* power_map = (map<int, int>*)map_[band];
		int last_power = 0;
		int last_drive = 0;
		float slope = 0.0;
		...
		for (; it != power_map->end() && it->first <= drive; it++) {
			...
			slope = (float)(it->second - last_power) / (float)(it->first - last_drive);
		}
		// We have another entry at higher drive so set the slope based on the before and after
		if (it != power_map->end()) {
			slope = (float)(it->second - last_power) / (float)(it->first - last_drive);
		}
		return last_power + (int)(slope * (float)(drive - last_drive));
	}
	else {
		return 0;
	}
...
map<int, int>* power_matrix::get_map(string band) {
	if (map_.find(band) != map_.end()) {
		return map_[band];
	}
	else {
