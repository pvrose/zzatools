...
void files_dialog::load_values() {
	...
	clublog_settings.get("Unzip Command", temp_string, "\"C://Program Files (x86)/7-Zip/7z\"");
...
void files_dialog::create_form(int X, int Y) {
	...
	in_unzipper->tooltip("Please enter executable to unzip");
	// Input - unzipper switches
	intl_input* in_switches = new intl_input(X + COL3, Y + ROW8_1, WBUTTON + GAP + WBUTTON, HTEXT, "Switches");
	...
	in_switches->tooltip("Please enter switches to unzip, use %s for filename and %s for directory");
	// Button - opens file browser
	Fl_Button* bn_browse_unzipper = new Fl_Button(X + COL5, Y + ROW8_1, WBUTTON, HBUTTON, "Browse");
	...
	bn_browse_unzipper->callback(cb_bn_browsefile, &unzipper_data_);
