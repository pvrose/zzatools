...
memory_table::memory_table(int X, int Y, int W, int H, const char* label, field_ordering_t order) : 
...
// DElete items 
void memory_table::delete_items() {
...
void memory_table::draw_cell(TableContext context, int R, int C, int X, int Y,
	int W, int H) {
	if (items_valid_) {
		switch (context) {
...
// OPen specific view
void memory_table::open_view(view_type type) {
...
// Open memory view
void memory_table::draw_memory_view() {
...
void memory_table::draw_scope_bands_view() {
...
void memory_table::draw_user_bands_view() {
...
void memory_table::draw_message_view(bool cw) {
	// Define table parameters
	rows(8);
	cols(2);
	items_ = new string * [rows()];
...
void memory_table::type(view_type type) {
...
view_type memory_table::type() {
	return type_;
}

void memory_table::resize_cols() {
	fl_font(FONT, FONT_SIZE);
	for (int c = 0; c < cols(); c++) {
		for (int r = 0; r < rows(); r++) {
			int w = 0;
...
int memory_table::handle(int event) {
	return Fl_Table_Row::handle(event);
}

// Null function
void memory_table::update(hint_t hint, unsigned int record_num_1, unsigned int record_num_2) {
	switch (hint) {
	case HT_MEMORIES:
		open_view((view_type)record_num_1);
		break;
	}
