...
, view()
{
	...
	edit_input_ = new intl_input(x() + w() / 2, y() + h() / 2, 0, 0, nullptr);
	edit_input_->hide();
		...
		case FL_KEYBOARD:
			...
			case FL_Tab:
				...
				if (Fl::event_state(FL_SHIFT)) {
					...
					that->done_edit();
					if (that->edit_col_ > 0) {
						that->edit_cell(that->edit_row_, ++that->edit_col_);
					}
				}
				else {
					// TAB - save and select cell to right
					that->done_edit();
					if (that->edit_col_ < that->fields_.size() - 1) {
						that->edit_cell(that->edit_row_, --that->edit_col_);
					}
				}
				break;
			...
			case FL_Down:
				...
				that->done_edit();
				if (that->edit_row_ < that->my_book_->size() - 1) {
					that->edit_cell(++that->edit_row_, that->edit_col_);
				}
				break;
			}
...
void log_table::edit_cell(int row, int col) {
	...
	edit_input_->show();
	edit_input_->resize(X, Y, W, H);				// Move Fl_Input widget there
	((intl_input*)edit_input_)->value(text.c_str());
	((intl_input*)edit_input_)->position(0, text.length());			// Select entire input field
	edit_input_->show();					// Show the input widget, now that we've positioned it
