...
enum file_contents_t : uint8_t {
	...
	FILE_FIELDS,                            //!< Field usage c=onfiguration file
...
const std::map < file_contents_t, file_control_t > FILE_CONTROL = {
	...
	{ FILE_COUNTRY_CFILES, { "cty.xml", true, false, DEBUG_RESET_CTY }},
	{ FILE_COUNTRY_DXATLAS, { "prefix.lst", true, false, DEBUG_RESET_CTY }},
	{ FILE_COUNTRY, { "cty.json", true, false, DEBUG_RESET_CTY } },
...
public:
	...
	//! Get directory
	std::string get_directory(bool code = false) {
		if (code) return default_code_directory_;
		else return default_data_directory_;
	}
