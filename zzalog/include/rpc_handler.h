...
class socket_server;
	...
	public:
		...
		rpc_handler(std::string host_address, int port_name, std::string resource_name);
		//! Destructor
		~rpc_handler();
	...
	protected:
		...
		//! \return true if successful.
		bool generate_request(std::string method_name, rpc_data_item::rpc_list* params, std::ostream& request_xml);
		...
		bool generate_response(bool fault, rpc_data_item* response, std::ostream& response_xml);
		...
		//! \return true if successful.
		bool decode_request(std::istream& request_xml, std::string& method_name, rpc_data_item::rpc_list* params);
