...
void ic7300_table::draw_memory_view() {
	...
	status_->misc_status(ST_NOTE, "RIG: Starting frequency fetch");
...
void ic7300_table::draw_user_bands_view() {
	...
	if (ok) {
		...
		for (int i = 0; i < num_userbands && ok; i++) {
			sub_command[0] = 3;
			status_->progress(i + num_txbands, OT_MEMORY);
		...
		for (int i = 0; i < num_txbands && ok; i++) {
			...
			*(items_ + r) = item;
			...
			double tx_lower = bcd_to_double(raw_tx_band.substr(1, 5), 6, true);
			double tx_upper = bcd_to_double(raw_tx_band.substr(7, 5), 6, true);
			...
			while (user_valid && user < num_userbands) {
				...
				if (raw_user_band[0] == '\xff' || raw_user_band.length() != 11) {
					...
					double user_lower = bcd_to_double(raw_user_band.substr(1, 5), 6, true);
					double user_upper = bcd_to_double(raw_user_band.substr(7, 5), 6, true);
					cout << "USER: lower = " << user_lower << "; upper = " << user_upper;
					if (user_lower >= tx_lower && user_upper <= tx_upper) {
						cout << " In current TX!" << endl;
						...
						// User band is not for the tx band - step tx band and row and not user
