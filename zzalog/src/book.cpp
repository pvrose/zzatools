...
void book::add_use_data(record* use_record) {
	...
	if (use_record->item("SWL") == "") {
		...
		if (get_macro(use_record, "APP_ZZA_QTH", qth_fields_, used_qths_, false)) {
			update_spec = true;
		}
		if (get_macro(use_record, "APP_ZZA_OP", op_fields_, used_ops_, true)) {
			update_spec = true;
		}
...
// Set the filename
void book::set_filename(string filename) {
	if (size()) {
		char msg[128];
...
// Get macro fields
set<string> book::get_macro_fields(string name) {
	if (name == "APP_ZZA_QTH") {
		return qth_fields_;
	} else if (name == "APP_ZZA_OP") {
		return op_fields_;
	} else {
		char msg[128];
		snprintf(msg, sizeof(msg),"LOG: Fields requested for unknown macro %s", name.c_str());
