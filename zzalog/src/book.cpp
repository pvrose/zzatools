...
void book::deprecate_macros(record* use_record) {
	...
	string qth_id = use_record->item("APP_ZZA_QTH");
	if (qth_id.length()) {
		// Copy items from ADIF record to STN data
		for (auto it : QTH_ADIF_MAP) {
			string value = use_record->item(it.second);
			if (value.length()) {
		...
		if (stn_items == nullptr) {
			...
			for (auto it : stn_items->data) {
				string item = QTH_ADIF_MAP.at(it.first);
	...
	if (oper_id.length()) {
		...
		if (stn_items == nullptr) {
			...
			for (auto it : stn_items->data) {
				string item = OPER_ADIF_MAP.at(it.first);
				use_record->item(item, it.second);
			}
		}
