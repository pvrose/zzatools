...
void dxa_if::set_dx_loc(string callsign) {
	...
	lat_long_t location = cty_data_->location(dx_record);
	if (!isnan(location.latitude) && !isnan(location.longitude)) {
		set_dx_loc(latlong_to_grid(location, 6), callsign);
	}
	else {
		// Not a DXCC entity - clear any existing DX Location
		snprintf(message, sizeof(message), "DXATLAS: Locating %s - DXCC entity has no location data", callsign.c_str());
		status_->misc_status(ST_WARNING, message);
		clear_dx_loc();
	}
}
