...
std::string spec_data::mode_for_submode(std::string submode) {
...
std::string spec_data::band_for_freq(double frequency_MHz) {
	...
	for (auto it = table->data.begin(); it != table->data.end() && !found; it++) {
		...
		double lower = std::stod(it->second->at("Lower Freq (MHz)"));
		double upper = std::stod(it->second->at("Upper Freq (MHz)"));
...
void spec_data::freq_for_band(std::string band, double& lower, double& upper) {
	...
	if (it != table->data.end()) {
		...
		upper = std::stod(it->second->at("Upper Freq (MHz)"));
	}
	else {
		// Else return NAN
		lower = nan("");
...
bool spec_data::is_submode(std::string mode) {
	// Get the Submode dataset
...
// Get named dataSet
spec_dataset* spec_data::dataset(std::string name) {
...
bool spec_data::add_userdef(int id, const std::string& name, char indicator, std::string& values) {
	...
	std::map<std::string, std::string>* temp_map = new std::map<std::string, std::string>;
	char temp[128];
	std::string userdef_name;
	(*temp_map)["Data Type"] = "String";
	(*temp_map)["Enumeration"] = "";
	sprintf(temp, "Header declaration of USERDEF%d", id);
	(*temp_map)["Description"] = std::string(temp);
	...
	if (indicator == 'E' && values.length() > 0) {
		...
		enumeration_name = std::string(temp);
...
bool spec_data::add_appdef(const std::string& name, char indicator) {
	...
	if (it_field != fields->data.end()) {
		...
		std::string data_type = it_field->second->at("Data Type");
...
std::string spec_data::datatype(char indicator) {
	// Get the Data Types dataset
	spec_dataset* data_types = dataset("Data Types");
...
std::string spec_data::datatype(std::string field_name) {
	// Get field dataset
	spec_dataset* fields = dataset("Fields");
	// get record
...
std::string spec_data::enumeration_name(const std::string& field_name, record* record) {
	// Get the Fields dataset
	spec_dataset* fields = dataset("Fields");
...
bool spec_data::add_user_enum(std::string field, std::string value) {
	...
	std::string enum_name = enumeration_name;
	...
	if (it_field != fields->data.end()) {
		...
		if (it_field->second->at("Data Type") == "Enumeration") {
			...
			original_data->data[field] = new std::map<std::string, std::string>(*(it_field->second));
	...
	if (types_dataset->data.find("Dynamic") == types_dataset->data.end()) {
		std::map<std::string, std::string>* dynamic_data = new std::map<std::string, std::string>;
	...
	if (enum_dataset->data.find(value) == enum_dataset->data.end()) {
		auto temp_map = new std::map<std::string, std::string>;
		(*temp_map)["Comments"] = "";
		(*temp_map)["ADIF Version"] = "";
		(*temp_map)["ADIF Status"] = "Not approved";
...
valn_error_t spec_data::check_number(const std::string&  data, const std::string&  field, const std::string&  datatype)
{
	...
	std::map<std::string, std::string>* fields = dataset("Fields")->data.at(field);
	double min_value = -DBL_MAX;
	if (fields->find("Minimum Value") != fields->end() && fields->at("Minimum Value").length()) {
		min_value = std::stod(fields->at("Minimum Value"));
	}
	double max_value = DBL_MAX;
	if (fields->find("Maximum Value") != fields->end() && fields->at("Maximum Value").length()) {
		max_value = std::stod(fields->at("Maximum Value"));
	}
...
valn_error_t spec_data::check_integer(const std::string&  data, const std::string&  field, const std::string&  datatype)
{
	...
	if (fields->find("Minimum Value") != fields->end()) {
		min_value = std::stoi(fields->at("Minimum Value"));
	}
	int max_value = INT_MAX;
	if (fields->find("Maximum Value") != fields->end()) {
		max_value = std::stoi(fields->at("Maximum Value"));
	}
...
valn_error_t spec_data::check_enumeration(const std::string& data, const std::string& field, const std::string& datatype) {
	...
	// BAND/BAND_RX should agree with FREQ/FREQ_RX (or the latter is "")
		...
		if (dataset(datatype)->data.find(data) != dataset(datatype)->data.end()) {
			...
			std::map<std::string, std::string>* fields = dataset(datatype)->data.at(data);
			// Get the frequency value from the record
			std::string frequency = (field == "BAND") ? record_->item("FREQ") : record_->item("FREQ_RX");
			if (frequency == "") {
				...
				std::string lower_edge = fields->at("Lower Freq (MHz)");
				std::string upper_edge = fields->at("Upper Freq (MHz)");
				double lower_value = std::stod(lower_edge);
				double upper_value = std::stod(upper_edge);
				size_t pos;
				double freq_value;
				try {
...
valn_error_t spec_data::check_string(const std::string&  data, const std::string&  field, const std::string&  datatype)
{
	...
	// SUBMODE - is only recommended to be one of an enumerated std::list
		...
		if (dataset("Submode")->data.find(upper_data) != dataset("Submode")->data.end()) {
			...
			std::string mode = fields->at("Mode");
			std::string record_mode = record_->item("MODE");
			if (mode == record_mode) {
				// They match - OK
				return VE_OK;
			}
		...
		std::string dxcc_code = field.length() == 10 ?
...
valn_error_t spec_data::check_time(const std::string& data, const std::string& field) {
	if (field == "TIME_OFF") {
...
valn_error_t spec_data::check_format(const std::string&  data, const std::string&  field, const std::string&  datatype, const std::basic_regex<char>& regex)
{
...
valn_error_t spec_data::check_datatype(const std::string&  data, const std::string&  field, const std::string&  datatype, bool is_enumeration)
{
	...
	if (is_enumeration) {
		...
		if (pos_dxcc != std::string::npos) {
			std::string dxcc_code_field;
		...
		if (enumeration_data->data.find(data) != enumeration_data->data.end()) {
			...
			if (enumeration_record->find("Import-only") == enumeration_record->end()) {
				...
				if (enumeration_record->find("Deleted Date") != enumeration_record->end()) {
					std::string deleted_date = enumeration_record->at("Deleted Date");
					if (deleted_date != "") {
	...
	// Test these data types in approximate order of frequency of occurrence
		...
		if (datatypes_set->data.find(datatype) != datatypes_set->data.end()) {
			std::map<std::string, std::string>* datatype_record = datatypes_set->data.at(datatype);
			// First check that it's not import only
			std::string import_only = "";
			...
			// Not ADIF type. Either a single Credit or a Credit, ':', ampersand seperated std::list of QSL_Medium
			std::string credit_granted = "Credit";
			// find the colon
...
valn_error_t spec_data::validate(const std::string& field, const std::string& data, bool inhibit_report /* = false */)
{
	...
	if (it != fields->data.end()) {
		std::map<std::string, std::string>* field_entry = it->second;
		if (data == "") {
...
// Handle the error
void spec_data::handle_error(valn_error_t error_code, const std::string&  data, const std::string&  datatype, const std::string&  field)
{
...
void spec_data::report_error(valn_error_t error_code, const std::string&  data, const std::string&  datatype, const std::string&  field)
{
	status_t error_level = ST_SEVERE;
	std::string incompat_field;
	// Get field information for some reports - only if it's a valid field
	std::map<std::string, std::string>* field_info = nullptr;
	if (error_code != VE_FIELD_UNKNOWN) {
		field_info = dataset("Fields")->data.at(field);
	}
	std::string output_line = "VALIDATE: ";
...
bool spec_data::auto_correction(valn_error_t error_code, const std::string&  data, const std::string& display_item, const std::string&  datatype, const std::string&  field)
{
	switch (error_code) {
	case VE_OK:
	...
	// Try again without trailing spaces
	{
		...
		if (pos != std::string::npos && pos != data.length()) {
			std::string no_trail = data.substr(0, pos + 1);
	...
	case VE_VALUE_INVALID:
		// Try again without trailing spaces
		{
			size_t pos = data.find_last_not_of(' ');
			if (pos != std::string::npos && pos != data.length()) {
				std::string no_trail = data.substr(0, pos);
				if (!check_datatype(no_trail, field, datatype, dataset(datatype) != nullptr)) {
					record_->item(field, no_trail);
					correction_message_ = field + '=' + display_item + " auto-corrected by removal of trailing spaces.";
					return true;
			...
			if (field == "MODE") {
				...
				if (it != submode_set->data.end()) {
					std::string mode = it->second->at("Mode");
	...
	case VE_VALUE_INPUT_ONLY:
		...
		if (field == "MODE") {
			...
			if (submode_set->data.find(data) != submode_set->data.end()) {
				...
				std::string mode = (*submode_data)["Mode"];
		...
		if (field == "QSL_RCVD" && data == "V") {
			...
			std::string credit_granted = record_->item("CREDIT_GRANTED");
			const std::string new_credit = "DXCC:card,DXCC_BAND:card,DXCC_MODE:card";
			if (credit_granted == "") {
		...
		if (field == "EQSL_QSL_RCVD" && data == "V") {
			// Value V no longer supported - changed to CREDIT_GRANTED
			std::string credit_granted = record_->item("CREDIT_GRANTED");
			const std::string new_credit = "EQSL:eqsl,EQSL_BAND:eqsl,EQSL_MODE:eqsl";
			if (credit_granted == "") {
				record_->item("CREDIT_GRANTED", new_credit);
			}
			else {
		...
		if (field == "STATE") {
			...
			case 61:
				...
				if (data == "FJL") {
					...
					record_->item(field, std::string("AR"));
			...
			case 151:
				...
				if (data == "MV") {
					...
					record_->item(field, std::string("LO"));
			...
			case 225:
				...
				if (data == "MD") {
					...
					record_->item(field, std::string("VS"));
		...
		if (field == "CONTEST_ID") {
			if (data == "EA-RTTY" || data == "RAC") {
				...
				const std::string new_item = "UKRAINIAN DX";
				...
				const std::string new_item = "VA-QSO-PARTY";
				record_->item(field, new_item);
				correction_message_ = field + "=" + display_item + " auto-corrected to " + field + "=" + new_item + ".";
				return true;
			}
	...
	case VE_VALUE_INCOMPATIBLE:
		if (field == "BAND") {
			...
			const std::string frequency = record_->item("FREQ");
			...
			if (difftime(new_off, time_on) > 0.0) {
				...
				record_->item("QSO_DATE_OFF", std::string(temp));
	...
	case VE_VALUE_OUTDATED:
		if (field == "ARRL SECT") {
			...
			if (data == "NWT") {
				...
				record_->item("ARRL_SECT", std::string("NT"));
...
// User-correct error
bool spec_data::ask_correction(const std::string& field)
{
...
// Report correction
void spec_data::report_correction(const std::string&  field, const std::string&  data, status_t status)
{
...
std::string spec_data::report_timestamp(std::string field, std::string data) {
	std::string timestamp = "QSO " +
...
std::string spec_data::get_tip(const std::string& field, record* record) {
	// Get the Fields dataset
	spec_dataset* fields = dataset("Fields");
	if (fields != nullptr) {
		...
		if (text == "Enumeration") {
			// Enumeration name
			std::string enumeration = enumeration_name(field, record).c_str();
...
std::string spec_data::get_tip(const std::string& field) {
	// Get the Fields dataset
	spec_dataset* fields = dataset("Fields");
	if (fields != nullptr) {
		// Build the tip up.
		...
		for (unsigned int i = 0; i < fields->column_names.size(); i++) {
			std::string& column = fields->column_names[i];
			std::string text = (*field_data)[column];
...
void spec_data::process_modes() {
	...
	for (auto it = modes->data.begin(); it != modes->data.end(); it++) {
		if (it->second->find("Input Only") == it->second->end()) {
			...
			(*data)[std::string("Is Mode")] = "Yes";
			(*data)[std::string("Mode")] = it->first;
	...
	for (auto it = submodes->data.begin(); it != submodes->data.end(); it++) {
		if (it->second->find("Input Only") == it->second->end()) {
			std::map<std::string, std::string>* data = new std::map<std::string, std::string>;
			(*data)[std::string("Is Mode")] = "No";
			(*data)[std::string("Mode")] = it->second->at("Mode");
			combined->data[it->first] = data;
		}
	}
...
std::string spec_data::summarise_enumaration(std::string name, std::string value) {
