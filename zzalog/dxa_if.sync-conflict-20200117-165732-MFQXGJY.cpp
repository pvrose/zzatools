...
HRESULT dxa_if::cb_map_clicked(float latitude, float longitude) {
	...
	if (qso_display_ != AQ_SEARCH && extract_records_) {
		...
		string s_lat = zzalib::degrees_to_dms(latitude, true);
		string s_lon = zzalib::degrees_to_dms(longitude, false);
		string gridsquare = zzalib::latlong_to_grid({ latitude, longitude }, 6);
...
HRESULT dxa_if::cb_mouse_moved(float latitude, float longitude) {
	if (call_layer_ != nullptr && !is_my_change_) {
		...
		DxAtlas::IDxMapPtr map = atlas_->GetMap();
		// We are going to change the map - let callbacks kzzalib::now and start building bulk change.
		is_my_change_ = true;
		map->BeginUpdate();
		...
		// We zzalib::now have the closest
		if (closest_num != -1) {
...
void dxa_if::home_location() {
	...
	home_lat_dms_ = zzalib::degrees_to_dms((float)home_lat_, true);
	home_long_dms_ = zzalib::degrees_to_dms((float)home_long_, false);
	// Add uncertainty
...
void dxa_if::initialise_map() {
	...
	int screen_w;
	int screen_h;
	// zzalib::now see if it really does intersect and by how much
...
void dxa_if::create_colour_buttons() {
	...
	// zzalib::now resize the window to just contain the buttons and don't allow the user to resize it smaller
...
void dxa_if::get_records() {
	...
	// zzalib::now see if QSOs, DXCCs or zones
		...
		try {
			...
			for (int colour_layer = 0; colour_layer < (signed)colours_used_.size(); colour_layer++) {
				...
				if (colour_enables_[colour_layer]) {
					...
					// zzalib::now put the data onto the DXATLAS: map and display an error if it failed
					try {
			...
			// zzalib::now add a layer for textual information - used for interactive displays
			...
			// zzalib::now centre on selected record
			if (map->GetProjection() == DxAtlas::PRJ_RECTANGULAR) {
				centre_map();
			}
...
void dxa_if::zoom_centre(lat_long_t centre, bool full) {
	...
	if (full) {
		...
		// zzalib::now zoom by the smaller of these with 5% margin
