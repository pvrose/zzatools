...
void files_dialog::load_values() {
	...
	clublog_settings.get("Unzip Command", temp_string, "\"C://Program Files (x86)/7-Zip/7z\" e %s -o%s -y");
...
void files_dialog::create_form(int X, int Y) {
	...
	const int XMAX = COL6 + WBUTTON + WLABEL + EDGE;
	...
	Fl_Spinner* spin_auto = new Fl_Spinner(X + COL6 + WLABEL, Y + ROW1_1, WBUTTON, HBUTTON, "Polling\nint. (secs)");
	...
	Fl_Group* grp_unzipper = new Fl_Group(X + XGRP, Y + GRP8, XMAX, HGRP8, "Unzipper command");
	grp_unzipper->labelsize(FONT_SIZE);
	grp_unzipper->box(FL_THIN_DOWN_BOX);
	grp_unzipper->align(FL_ALIGN_LEFT | FL_ALIGN_TOP | FL_ALIGN_INSIDE);
	// Input - status log file name
	intl_input* in_unzipper = new intl_input(X + COL2, Y + ROW7_1, WEDIT + GAP + WBUTTON + GAP + WBUTTON, HTEXT);
	...
	in_unzipper->tooltip("Command to unzip - add position of filename and output switxh/directpry");
	// Button - opens file browser
	Fl_Button* bn_browse_unzipper = new Fl_Button(X + COL5, Y + ROW7_1, WBUTTON, HBUTTON, "Browse");
	bn_browse_unzipper->align(FL_ALIGN_INSIDE);
	unzipper_data_ = { "Please enter the unzipper executable", "Executable (*exe)|*.exe|All Files (*.*)|*.*||", &unzipper_, nullptr, in_unzipper, nullptr };
	bn_browse_unzipper->callback(cb_bn_browsefile, &status_data_);
	bn_browse_unzipper->when(FL_WHEN_RELEASE);
	bn_browse_unzipper->labelsize(FONT_SIZE);
	bn_browse_unzipper->tooltip("Opens a file browsewr to locate the unzipper executable");
