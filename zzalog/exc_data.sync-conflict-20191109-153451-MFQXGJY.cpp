...
exc_entry* exc_data::is_exception(record* record) {
	...
	if (entries_.find(call) == entries_.end()) {
		...
		for (auto it = list_entries->begin(); it != list_entries->begin(); it++) {
			exc_entry* entry = *it;
			if ((entry->end == -1 || entry->end > timestamp) && 
				(entry->start == -1 || timestamp > entry->start)) {
				char message[160];
				snprintf(message, 160, "EXCEPTION: Contact %s at %s %s is in entity %s", call.c_str(), record->item("QSO_DATE").c_str(), record->item("TIME_ON"), entry->entity.c_str());
